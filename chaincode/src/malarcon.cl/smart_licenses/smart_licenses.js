...
const initalLicensers = [
    {
        id: '1',
        name: "Licenser #1",
        companyName: "Licenser LTDA",
        accountType: "LICENSER",
        status: 'ACTIVE',
        licenses: [
            {
                id: '1',
...
let SmartLicenses = class {
	...
	async Init(stub) {
		logger = shim.newLogger('NODE_SMART_LICENSES');
		logger.level = 'debug';

		logger.info('=========== Instantiated smart licenses chaincode ===========');
		return shim.success();
	}
    ...
    async initLedger(stub, args) {
		...
		for (const licenser of initalLicensers) {
			
			await stub.putState(licenser.id, Buffer.from(JSON.stringify(licenser)));
			logger.info('Added <--> ', licenser);
		}

		logger.info('============= END : Initialize Ledger ===========');
	}

	async queryAllLicensers(stub, args) {
    ...
    async createLicenser(stub, args) {
        logger.info('============= START : Create Customer ===========');

		let licenser = JSON.parse(args[0]);
    ...
    async addLicensesToLicenser(stub, args) {
        logger.info('============= START : Create Customer ===========');

        const request = JSON.parse(args[0]);
		let licenserAsBytes = await stub.getState(request.id);
		let licenser = JSON.parse(licenserAsBytes);
        licenser.licenses = licenser.licenses.concat(request.licenses);
		await stub.putState(licenser.id, Buffer.from(JSON.stringify(licenser)));
		logger.info('============= END : Create Customer ===========');
	}
